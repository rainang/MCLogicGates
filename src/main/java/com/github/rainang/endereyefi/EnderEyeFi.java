...
public class EnderEyeFi {
	...
	public void init(FMLInitializationEvent event) {
		...
		GameRegistry
				.addShapedRecipe(new ItemStack(EnderBlocks.REPEATERS[0][3][1], 1), "#X#", "III", '#', Items
						.ender_pearl,
								 'X',
								 Items.redstone, 'I', stackStone);
		GameRegistry.addShapedRecipe(new ItemStack(EnderBlocks.REPEATERS[0][2][1], 1), "RX#", "III", 'R',
									 Blocks.redstone_torch, 'X', Items.redstone, '#', Items.ender_pearl, 'I',
									 stackStone);
		GameRegistry
				.addShapelessRecipe(new ItemStack(EnderBlocks.REPEATERS[0][1][1], 1), EnderBlocks.REPEATERS[0][2][1]);
		GameRegistry
				.addShapelessRecipe(new ItemStack(EnderBlocks.REPEATERS[0][2][1], 1), EnderBlocks.REPEATERS[0][1][1]);
		...
		GameRegistry.addShapedRecipe(new ItemStack(EnderBlocks.CASTERS[2][0][0], 6), "#X#", "#E#", "#Y#", 'E',
									 Items.ender_eye, '#', Blocks.obsidian, 'Y', Items.ender_pearl, 'X',
	...
	public void subscribe(PlayerInteractEvent event) {
		if(event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
			...
			if(Blocks.unpowered_repeater == state.getBlock())
				event.world.setBlockState(event.pos, EnderBlocks.REPEATERS[0][0][1].getDefaultState().withProperty(
						BlockDirectional.FACING,
																					   state.getValue(
																							   BlockDirectional
																									   .FACING)));
			else if(Blocks.powered_repeater == state.getBlock())
				event.world.setBlockState(event.pos, EnderBlocks.REPEATERS[1][0][1].getDefaultState().withProperty(
						BlockDirectional.FACING,
																					   state.getValue(
																							   BlockDirectional
																									   .FACING)));
